    ...
    render()
    {
        ...
        const pagination =()=> {
            ...
            if (max < 11) {
                for (var i = 1; i <= max; i++) {
                    if (i === current) {
                        paginationItems.push(<Pagination.Item active>{i}</Pagination.Item>);
                    } else {
                        const index = i;
                        paginationItems.push(<Pagination.Item onClick={()=>this.props.indexClick(index)}>{index}</Pagination.Item>);
                    }
                }
            } else {
                var tempIndex = 1;
                if (current > 5) {
                    paginationItems.push(<Pagination.Item onClick={()=>this.props.indexClick(1)}>{1}</Pagination.Item>);
                ...
                paginationItems.push(<Pagination.Item active>{current}</Pagination.Item>);
