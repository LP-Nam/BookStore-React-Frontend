...
class UpdateBookType extends Component {
    constructor(props){
        super(props)
        this.state ={
            items: [{}]
    ...
    componentDidMount(){
        fetch(`http://localhost:3001/api/admin/findByBookType/${this.props.match.params.id}`)
        .then(res => res.json())
        .then(
            (result) => {
                console.log(result);
    ...
    handleSubmit = (e)=>{
        e.preventDefault()
        fetch(`http://localhost:3001/api/admin/UpdateBookType/${this.props.match.params.id}`, {
            method: 'PUT',
            headers: {
