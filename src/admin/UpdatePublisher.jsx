...
class UpdatePublisher extends Component {
    ...
    componentDidMount(){
        fetch(`http://localhost:3001/api/admin/findByPublisherAdmin/${this.props.match.params.id}`)
        .then(res => res.json())
        .then(
            (result) => {
                this.setState({
    ...
    handleSubmit = (e)=>{
        e.preventDefault()
        fetch(`http://localhost:3001/api/admin/UpdatePublisher/${this.props.match.params.id}`, {
            method: 'PUT',
            headers: {
